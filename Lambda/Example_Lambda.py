...
# 下面的网格生成、模型设置、装置设置等代码整合到一个py文件中放在本地，在本地运行和生成，然后将生成的结果做为函数输入通过API传给AWS Lambda
...
# 下面这段是需要放在云上，由AWS Lambda执行的，可以把下面这段封装成一个函数，这个函数的输入变量是nodeX、nodeY、nodeZ、cellCon、faceCon、edgeCon、tx、rx
# 通过API将上述参数传给AWS Lambda，再由AWS计算后，返回data做为输出
...
# 需要由AWS Lambda执行的代码到此为止，Lambda再通过API将变量data返回给调用者

# 下面的代码留在本地（可以与前面的网格设计、模型设计等一起放一个文件），不在Lambda上运行。由API返回data变量后，在本地用data里面的数据画图
