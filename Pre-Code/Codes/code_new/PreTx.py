...
def process_tx_id(tx_id, params, minutes_of_action, current_hour):
    ...
    for action_minute in tqdm(minutes_of_action, desc=f"Processing TX_ID {tx_id}"):
        ...
        pattern = os.path.join(params['INPUT_PATH'], f'{tx_id}.{start_time.strftime("%Y%m%d.%H%M")}*0000.Z.miniseed')
...
def main():
    while True:
        try:
            ...
            if not params:
                print("读取预处理参数失败。正在退出。")
                time.sleep(60)
                continue

            # 确保 'INPUT_PATH' 在参数中
            if 'INPUT_PATH' not in params:
                print("错误：参数中未找到 'INPUT_PATH'。正在退出。")
                time.sleep(60)
                continue
            ...
            print(f"操作分钟数: {minutes_of_action}")
            ...
            with ProcessPoolExecutor() as executor:
                ...
                problem_files = []
                for future in tqdm(futures, desc="总体进度"):
